...
#       (sbatch --wait --array=1-N%4 Step04a_STAR_Chimeric_Align.sh),
...
# This is the missing generation stage: the junctions consumed by Step04b are
...
# applied later, at count time, in Step04b. So these junction files are valid
...
if [ "${STAR_VERSION}" != "${REQUIRED_STAR_VERSION}" ]; then
    ...
    echo "       Step04b parses are version-specific. Refusing to run."
    exit 1
fi
