...
recomputed here. Gene/protein names are drawn plain (protein context).

Building blocks:
  draw_binding_bars       horizontal STACKED bars: wild-type baseline binding +
                          the mutation's binding gain, per differential neoantigen.
  draw_gene_track         native-HGVS-frame backbone, domain boxes, disordered
                          underlay, TCW-colored lollipops.
  draw_expr_carrier_stacked  vertical stacked bars: % cells expressing with the
                          mutation-carrying proportion stacked inside.
  draw_panelA             per-cell per-UMI burden (neoantigen + fusion), SEM, adj p.
  draw_panelB_venn        neoantigen gene overlap (SBS2 / shared / CNV).
...
WT_GRAY = '#D0D3D4'        # wild-type baseline segment / expressing segment
...
# RENDERERS
# =============================================================================
def draw_binding_bars(ax, gene, binding, panel=None):
    """Horizontal stacked bars per differential neoantigen mutation:
    wild-type baseline binding score, then the mutation's binding gain stacked on
    top (colored by TCW class). The colored segment is the neoantigen created."""
    title = f"{gene}  MHC-I binding gain"
    if panel:
        title = f"{panel}  " + title
    ax.set_title(title, fontsize=FS_LABEL, pad=10, loc='left')
    if len(binding) == 0:
    ...
    for y, row in zip(ys, binding.itertuples()):
        ...
        ax.barh(y, s_wt, height=0.60, color=WT_GRAY, edgecolor='#333333',
                linewidth=1.3, zorder=2)
        ax.barh(y, max(s_mut - s_wt, 0), left=s_wt, height=0.60, color=col,
                edgecolor='#333333', linewidth=1.3, zorder=3)
        ax.text(s_mut + 0.015, y, f"{row.mut_peptide}", va='center', ha='left',
    ...
    ax.text(THRESH_SCORE, n - 0.30, 'binder\n(500 nM)', ha='center', va='bottom',
...
def draw_gene_track(ax, gene, track, lollipops, panel=None, legend='disorder'):
    """Native-frame backbone with functional domain boxes, a muted disordered-region
    underlay, and TCW-colored mutation lollipops. `legend`: 'disorder' shows just the
    disordered-region key (TCW colors live in the shared binding legend on combined
    figures); 'full' also keys the lollipop colors (for standalone track panels);
    None draws no legend."""
    ...
    if lollipops:
        ...
        for i, lp in enumerate(lollipops):
            ...
            ax.scatter([x], [bar_y + bar_h + 1.4], s=560 if i == anchor else 340,
                       facecolors=lp['color'], edgecolors='#222222', linewidths=2.4,
                       zorder=6)
    label = f"{gene}  ({track['acc']}, {L} aa)"
    if panel:
        label = f"{panel}  " + label
    ax.text(0, bar_y + bar_h + 2.2, label, fontsize=FS_LABEL - 2, va='bottom')
    # legend keying the disordered underlay (and optionally the mutation colors)
    if legend:
        handles = []
        if track['disordered']:
        ...
        if handles:
            ax.legend(handles=handles, loc='lower right', ncol=1,
                      fontsize=FS_SMALL - 4, frameon=False)
    ax.set_yticks([])
...
def draw_expr_carrier_stacked(ax, genes, panel='A'):
    """Vertical stacked bars: bar height = % of cells expressing the gene; the
    coral sub-segment is the fraction of ALL cells that both express and carry a
    mutation. The per-bar annotation gives the carrier fraction among expressers."""
    data = [(g, load_expression_carrier(g)) for g in genes]
    data = [(g, e) for g, e in data if e]
    xs = np.arange(len(data))
    for x, (g, e) in zip(xs, data):
        expr = e['headline_pct']
        carrier_all = expr * e['carrier_pct'] / 100.0
        ax.bar(x, carrier_all, width=0.62, color=COLOR_SBS2, edgecolor='#333333',
               linewidth=1.4, zorder=3)
        ax.bar(x, expr - carrier_all, bottom=carrier_all, width=0.62, color=WT_GRAY,
               edgecolor='#333333', linewidth=1.4, zorder=2)
        ax.text(x, expr + 1.5, f"{expr:.0f}%", ha='center', va='bottom', fontsize=FS_SMALL - 2)
        ax.text(x, max(carrier_all + 1.5, 4.0), f"{e['carrier_pct']:.1f}%", ha='center',
                va='bottom', fontsize=FS_SMALL - 6, color='#7B241C')
    ax.set_xticks(xs)
    ax.set_xticklabels([g for g, _ in data], fontsize=FS_TICK - 2)
    ax.set_ylabel('% of cells expressing', fontsize=FS_TICK - 2)
    ymax = max(e['headline_pct'] for _, e in data) if data else 1
    ax.set_ylim(0, ymax * 1.22)
    ax.tick_params(labelsize=FS_TICK - 4)
    ax.legend(handles=[Patch(facecolor=WT_GRAY, edgecolor='#333', label='expressing'),
                       Patch(facecolor=COLOR_SBS2, edgecolor='#333',
                             label='expressing + carries mutation')],
              fontsize=FS_SMALL - 2, frameon=False, loc='upper right')
    for s in ('top', 'right'):
        ax.spines[s].set_visible(False)
    ttl = "expression and neoantigen carriage among expressing cells"
    ax.set_title((f"{panel}  " if panel else "") + ttl, fontsize=FS_LABEL, loc='left', pad=10)


def make_bt_axes(fig, cell):
    """Binding (left) + track (right) for one supplemental gene."""
    gs = GridSpecFromSubplotSpec(1, 2, subplot_spec=cell, width_ratios=[0.48, 0.52],
                                 wspace=0.30)
    return fig.add_subplot(gs[0, 0]), fig.add_subplot(gs[0, 1])


def draw_bt_card(fig, cell, card, panel=None):
    ab, at = make_bt_axes(fig, cell)
    draw_binding_bars(ab, card['gene'], card['binding'], panel=panel)
    draw_gene_track(at, card['gene'], card['track'], card['lollipops'])
...
def draw_panelB_venn(ax, counts, panel='B'):
    ...
    ax.set_title((f"{panel}  " if panel else "") + 'neoantigen gene overlap',
