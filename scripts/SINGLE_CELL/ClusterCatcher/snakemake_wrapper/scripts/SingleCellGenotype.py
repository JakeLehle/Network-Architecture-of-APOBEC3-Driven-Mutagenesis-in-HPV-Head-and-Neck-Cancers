...
def run_interval(code,var_dict,meta_dict,BAM,FASTA,tmp_dir,BQ,MQ,MAX_DP,ALT_FLAG):
    ...
    for p in i:
        ...
        if POS in List_of_target_sites:
            ...
            # NEW: First pass - calculate total depth per cell barcode at this position
            CELL_TOTAL_DEPTH = {}
            for read_i in range(len(reads)):
                ...
                # Check if barcode is in metadata
                try:
                    CELL_TYPE = meta_dict[barcode]
                except:
                    continue

                # Only count high-quality reads for total depth
                if (read_alignment.is_secondary == False and
                    read_alignment.is_duplicate == False and
                    read_alignment.is_supplementary == False):

                    if barcode in CELL_TOTAL_DEPTH:
                        CELL_TOTAL_DEPTH[barcode] += 1
                    else:
                        CELL_TOTAL_DEPTH[barcode] = 1
            ...
            # Second pass - count reads per base per cell
            ...
            # Write output with total depth included
            for group in CELLS.keys():
                barcode,cell_type,Alt_obs = group.split('>>')
                total_depth = CELL_TOTAL_DEPTH.get(barcode, 0)  # Get total depth for this cell

                Group = [str(CHROM), str(POS+1), str(POS+1), Ref_exp, Alt_exp, str(Cell_type_exp),
                        str(Num_cells_exp), barcode, cell_type, Alt_obs, str(CELLS[group]), str(total_depth)]
                Group = '\t'.join(Group) + '\n'
                outfile.write(Group)
...
def concatenate_sort_temp_files_and_write(out_file, tmp_dir):
    ...
    if (len(all_files) > 0):
        ...
        out = open(out_file,'w')
        Header=['#CHROM','Start', 'End', 'REF', 'ALT_expected', 'Cell_type_expected','Num_cells_expected','CB','Cell_type_observed','Base_observed','Num_reads','Total_depth']
