...
for cancer_type in CANCER_TYPES:
    ...
    if HAS_GSEAPY and partition_df is not None:
        ...
        for i, c in enumerate(sorted_comms):
            ...
            results_df, err = enrichr_with_retry(genes_symbols_clean)
            ...
            if results_df is not None and len(results_df) > 0:
                ...
                log(f"    [ERROR] Enrichment failed: {err}")
                enrichment_results.append({
                    "community": c,
                    "n_genes": n_total,
                    "n_mapped": n_mapped,
                    "top_kegg_term": f"ERROR: {str(err)[:50]}",
                    "top_kegg_pvalue": None,
                    "top_kegg_genes": None,
                    "n_kegg_significant": 0
                })

            # Rate-limit delay between communities (skip after last one)
