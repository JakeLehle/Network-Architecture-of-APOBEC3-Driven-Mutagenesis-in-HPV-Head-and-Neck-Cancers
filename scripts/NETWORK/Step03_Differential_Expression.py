...
Workflow (mirrors single-cell pipeline):
  ...
  7. BH-FDR correction
  8. Select genes: raw p < 0.05 AND |logFC| > threshold
  9. Force-keep A3A and A3B only (not full A3 family)
...
Note on force-keep: A3A and A3B cannot be differentially expressed in this
design because the group definition controls for summed A3A+A3B expression.
They are force-kept specifically to observe which network communities they
associate with. This is a documented limitation of the bulk approach.
...
# A3 FORCE-KEEP SCOPE
# =============================================================================
# Only A3A and A3B are force-kept. The full A3 family (A3C/D/F/G/H) is NOT
# force-kept because they are not the primary drivers of SBS2 mutagenesis
# and adding them inflates the network without adding interpretive value.
# =============================================================================
A3_FORCE_KEEP_GENES = [
    "ENSG00000128383",  # A3A
    "ENSG00000179750",  # A3B
...
def bh_fdr(pvals):
    ...
    # Monotonicity: cumulative min from right to left
...
for cancer_type in CANCER_TYPES:
    ...
    # Force-keep A3A and A3B only (not full family)
    if FORCE_KEEP_A3:
        for g in A3_FORCE_KEEP_GENES:
            if g in protein_coding_genes and g not in genes_passing:
                genes_passing.append(g)
                sym = ensg_to_symbol.get(g, g)
                log(f"[STEP 12.4] Force-kept low-expression gene: {sym} ({g})")
    ...
    # Report both raw and FDR counts
    ...
    if FORCE_KEEP_A3:
        for g in A3_FORCE_KEEP_GENES:
            if g in stats_df["gene"].values and g not in selected:
                ...
                log(f"[STEP 12.8] Force-kept: {sym} "
                    f"(raw_p={row['p_value']:.4f}, adj_p={row['p_adj']:.4f}, logFC={row['logFC']:.3f})")
    ...
    print(f"  Significant (FDR < {FDR_THRESHOLD}): {n_sig_fdr}")
    print(f"  Selected (+ force-kept A3A/A3B): {len(selected)}")
