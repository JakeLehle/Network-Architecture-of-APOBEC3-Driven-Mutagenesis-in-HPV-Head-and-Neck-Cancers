...
Purpose
-------
Second pass. CURRENT_PANEL now reflects the first audit's accepted decisions
(APOBEC/Innate Immune block dropped, A3A/A3B stay in Panel B; KRT1 dropped;
BARD1/TP53BP1/RIF1/NSD2 promoted into the DDR block; STAT5A/STAT5B kept as the
HPV-specific STAT5->ATM activators). CANDIDATE_ADDITIONS is now the early /
maintenance end of the lifecycle that the current panel underrepresents: an
interferon / antiviral effector set and an HPV establishment/maintenance +
SMC5/6 restriction set, both predicted to lean SBS2-HIGH (and NORMAL) rather
than ride the productive CNV-HIGH wave. For every gene the audit reports whether
it (a) pulls its weight and (b) sits in the right category, with the peak-
population test the key readout for whether the early sets land. Nothing here
edits the figure generator; it writes a table + report we use to revise by hand.
...
# GENE PANELS
# =============================================================================
# Current Panel C AFTER the first audit's accepted decisions:
#   - APOBEC/Innate Immune block dropped entirely (A3A/A3B live in Panel B)
#   - KRT1 dropped from Differentiation (dead: max frac 0.01)
#   - BARD1/TP53BP1/RIF1/NSD2 promoted into the DDR block (BRD4-recruited arm)
#   - STAT5A/STAT5B kept in the DDR block as the HPV-specific STAT5->ATM
#     activators (Hong & Laimins); block reframed as ATM-dependent DDR with its
#     STAT5 activators and the BRD4-recruited arm.
CURRENT_PANEL = OrderedDict([
    ('Immune Signaling',
        ['STAT1', 'HLA-A', 'HLA-B', 'HLA-C', 'IRF1', 'TAP1', 'B2M']),
    ('Differentiation',
        ['KRT5', 'KRT14', 'KRT10', 'CDH1', 'IVL']),
    ('ATM/DNA Damage',
        ['ATM', 'CHEK2', 'CHEK1', 'BRCA1', 'MRE11', 'RAD50', 'NBN', 'H2AX',
         'STAT5A', 'STAT5B', 'BARD1', 'TP53BP1', 'RIF1', 'NSD2']),
...
# Candidate additions under evaluation in THIS pass: the early / maintenance
# end of the lifecycle, which the current panel underrepresents (it is
# productive-heavy). Predicted to lean SBS2-HIGH (maintenance) and/or NORMAL.
# BRD4 is already in Transformation, so it is not repeated below.
CANDIDATE_ADDITIONS = OrderedDict([
    # Interferon / antiviral effectors. Prediction: rise-then-fall across the
    # axis (low NORMAL -> up SBS2-HIGH maintenance/immune-visible -> down
    # CNV-HIGH productive/evaded). MX1 also appeared in the Fig 5 HC-variant set.
    ('Interferon / Antiviral (early?)',
        ['MX1', 'ISG15', 'OAS1', 'IFIT1', 'IFI27', 'IFI44L',
         'IFITM1', 'RSAD2', 'STAT2', 'IRF9', 'DDX58', 'BST2']),
    # HPV establishment / maintenance + the SMC5/6 restriction complex the virus
    # antagonizes. Prediction: maintenance-leaning (SBS2-HIGH), with the caveat
    # that maintenance host biology overlaps the productive machinery, so some
    # may ride the proliferation wave and peak CNV-HIGH instead.
    ('HPV maintenance / restriction (early?)',
        ['TOPBP1', 'SMC5', 'SMC6', 'NSMCE2']),
])
...
EXPECTED_PEAK = {
    ...
    # early tie-in predictions under test:
    'Interferon / Antiviral (early?)':        'SBS2_HIGH',  # rise-then-fall, peaks SBS2-HIGH
    'HPV maintenance / restriction (early?)': 'SBS2_HIGH',  # maintenance-leaning (some may ride CNV)
}
...
ALIASES = {
    ...
    'DDX58':   ['DDX58', 'RIGI', 'RIG-I'],
    'ISG15':   ['ISG15', 'G1P2'],
    'IRF9':    ['IRF9', 'ISGF3G'],
    'RSAD2':   ['RSAD2', 'CIG5'],
    'IFI27':   ['IFI27', 'ISG12'],
    'BST2':    ['BST2', 'CD317', 'TETHERIN'],
    'IFITM1':  ['IFITM1', 'CD225'],
    # HPV maintenance / restriction
    'NSMCE2':  ['NSMCE2', 'NSE2', 'MMS21'],
}
